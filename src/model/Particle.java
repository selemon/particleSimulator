...
public class Particle {
  ...
  public double x;
  public double y;
  public void move(ModelParallel modelParallel){
    x+=speedX/(ModelParallel.timeFrame);
    y+=speedY/(ModelParallel.timeFrame);
    //uncomment the following to have particle bouncing on the boundary
    if(this.x<0){this.speedX*=-1;}
    if(this.y<0){this.speedY*=-1;}
    if(this.x>ModelParallel.size){this.speedX*=-1;}
    if(this.y>ModelParallel.size){this.speedY*=-1;}
  }
  ...
  public void interact(ModelParallel modelParallel){
    for(Particle p:modelParallel.p){
      if(p==this)continue;
      ...
      dirX=p.mass*ModelParallel.gravitationalConstant*dirX/dist;
      dirY=p.mass*ModelParallel.gravitationalConstant*dirY/dist;
      assert this.speedX<=ModelParallel.lightSpeed:this.speedX;
      assert this.speedY<=ModelParallel.lightSpeed:this.speedY;
      double newSpeedX=this.speedX+dirX;
      newSpeedX/=(1+(this.speedX*dirX)/ModelParallel.lightSpeed);
      double newSpeedY=this.speedY+dirY;
      newSpeedY/=(1+(this.speedY*dirY)/ModelParallel.lightSpeed);
